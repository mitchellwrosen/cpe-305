...
with open('Othello.0.gen', 'w') as genfile:
  ...
  for command in ['enterMove', 'saveBoard', 'loadBoard', 'saveMove',
                  'loadMove', 'compareKeys', 'compareMove', 'undoLastMove',
                  'setOptions\n']:
    genfile.write('%s %s\n' % (command, 'a' * 5000))
  genfile.write('testRun 1 5000\nundoLastMove 9999999999999999999999')
  ...
  for i in xrange(runs):
    genfile.write('testRun %s 10\n' % (i * 1324))
    genfile.write('saveBoard b%d\n' % i)
    genfile.write('saveMove m%d\n' % i)

  for i in xrange(runs):
    genfile.write('loadMove m%d\n' % i)
    genfile.write('showMove\n')
    genfile.write('loadBoard b%d\n' % i)
    for j in xrange(runs):
      genfile.write('compareBoard b%d\n' % j)
...
# Othello.3.gen
###########################################################
with open('Othello.3.gen', 'w') as genfile:
  genfile.write('\n# Play quick games\n')
  games = 14
  for i in xrange(games):
    genfile.write('showAll\n')
